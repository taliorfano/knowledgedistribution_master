...
for l in range(totalLanguages): 
    ...
    repos = requests.get('https://api.github.com/search/repositories?q=bootstrap&language:' + language+'&sort=' + sort + '&order=' + order + '&per_page=1')
    ...
    for i in range(reposSize):
        ...
        with open('coletas-bootstrap.csv', 'w', newline='', encoding='utf-8') as csvfile:
            ...
            while(page<=480):
                ...
                for c in range(commitsSize):
                    ...
                    result = db.bootstrap.insert_one(contributions)
