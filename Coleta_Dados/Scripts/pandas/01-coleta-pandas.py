...
# Obtem dados de todas as linguagens analisadas e seus respectivos projetos, na ordem de relevancia
for l in range(totalLanguages): 
    ...
    repos = requests.get('https://api.github.com/search/repositories?q=pandas&language:' + language+'&sort=' + sort + '&order=' + order + '&per_page=1')
    ...
    for i in range(reposSize):
        ...
        #Calcular quantos contribuidores tem no repositorio e iterar o total de paginas: total_contribuidores/100
        page = 1 
        
        with open('coletas-pandas.csv', 'w', newline='', encoding='utf-8') as csvfile:
            ...
            while(page<=210):
                ...
                for c in range(commitsSize):
                    ...
                    result = db.pandas.insert_one(contributions)
